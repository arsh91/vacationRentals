...
if(isset($_POST['TicketNum']) && isset($_POST['eta_radio']) && isset($_POST['teamMemberId'])){
           ...
           $assignedTeamMemberName= $teamMemberName['Fname']." ".$teamMemberName['Lname'];
            ...
            $doorCode = $PropertyAddress['DoorCode'];
                ...
                $bodytext = "<p>Our team member, ".$assignedTeamMemberName." has responded to your request for assistance for Ticket Number ".$ticketId.", ".$maintenancecategory.".</p><p>".$teamMemberName['Fname']." has stated that he expects to resolve this issue by ".date("m-d-Y", strtotime($ETA['ETADate']) )." at ".date("h:i A", strtotime($ETA['ETATime']) )." .</p>";
